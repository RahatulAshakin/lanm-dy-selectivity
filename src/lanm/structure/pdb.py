...
def _format_atom_line(serial: int, atom: AtomRecord) -> str:
    ...
    return (
        f"{'ATOM':<6}{serial:>5} {atom_name}{' ':1}{atom.residue_name:>3} "
...
def render_protein_pdb(
    ...
    *,
    ...
    filtered_atoms = [
        atom
        for atom in atoms
        if atom.record_type == "ATOM" and atom.chain_id in chain_order
    ]
    selected_atoms = select_preferred_atom_conformers(filtered_atoms)
    ordered_atoms = sorted(
        selected_atoms,
        key=lambda atom: (
            chain_order[atom.chain_id],
            atom.residue_seq,
            atom.insertion_code,
            atom.atom_serial,
            atom.atom_name,
    ...
    lines: list[str] = []
    serial = 1
    atoms_by_chain: dict[str, list[AtomRecord]] = {chain_id: [] for chain_id in selected_chain_ids}
    ...
    for chain_id in selected_chain_ids:
        ...
        for atom in chain_atoms:
            ...
            serial += 1
        lines.append(_format_ter_line(serial, chain_atoms[-1]))
        serial += 1
    lines.append("END")
    return "\n".join(lines) + "\n"
