...
from lanm.analysis.ligandmpnn_round2_smoke import (
    LigandMPNNRound2SmokeCandidate,
    build_round2_ligandmpnn_run_command,
    discover_round2_ligandmpnn_smoke_candidates,
    parse_ligandmpnn_fasta,
    render_ligandmpnn_round2_smoke_markdown,
    summarize_round2_ligandmpnn_output,
)
from lanm.models import AtomRecord
from lanm.structure.pdb import render_selected_structure_pdb
...
def _write_test_round2_pdb(
    path: Path,
    *,
    metal_identity: str,
    solvent_residue_count: int,
) -> None:
    polymer_atom = AtomRecord(
    ...
    metal_atom = AtomRecord(
        ...
        record_type="HETATM",
        atom_serial=2,
        atom_name=metal_identity,
        alt_loc="",
        residue_name=metal_identity,
        chain_id="Z",
        residue_seq=1,
        insertion_code="",
        x=1.0,
    ...
    water_atoms = [
        ...
        for index in range(solvent_residue_count)
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        render_selected_structure_pdb(
            (polymer_atom, metal_atom, *water_atoms),
            selected_chain_ids=("A",),
            included_het_residue_keys=frozenset(
                atom.residue_key for atom in (metal_atom, *water_atoms)
            ),
        ),
        encoding="utf-8",
    )
...
def test_discover_round2_ligandmpnn_smoke_candidates_reads_manifest_and_derives_context(tmp_path: Path) -> None:
    inputs_dir = tmp_path / "inputs"
    output_root = tmp_path / "outputs"

    alpha_dir = inputs_dir / "alpha_candidate"
    alpha_pdb = alpha_dir / "alpha_candidate.pdb"
    _write_test_round2_pdb(alpha_pdb, metal_identity="ND", solvent_residue_count=1)
    (alpha_dir / "redesigned_residues.txt").write_text("A35 C8\n", encoding="utf-8")

    beta_dir = inputs_dir / "beta_candidate"
    beta_pdb = beta_dir / "beta_candidate.pdb"
    _write_test_round2_pdb(beta_pdb, metal_identity="DY", solvent_residue_count=0)
    ...
    assert candidates[0].seed_candidate_id == "alpha_seed"
    assert candidates[0].designed_chains == ("A", "C")
    assert candidates[0].preserved_metal_identity == "ND"
    assert candidates[0].preserved_solvent_residue_count == 1
    assert candidates[0].redesigned_residue_ids == ("A35", "C8")
    assert candidates[0].output_dir == output_root / "alpha_candidate"
    assert candidates[1].fixed_context_chains == ("B",)
    assert candidates[1].preserved_metal_identity == "DY"
    assert candidates[1].preserved_solvent_residue_count == 0


def test_build_round2_ligandmpnn_run_command() -> None:
    candidate = LigandMPNNRound2SmokeCandidate(
        shortlist_rank=1,
        candidate_id="example_candidate",
        seed_rank=1,
        seed_candidate_id="example_seed",
        campaign_id="example_campaign",
        backbone_id="example_backbone",
        topology_class="example_topology",
        source_pdb_path=Path("/tmp/example_seed.pdb"),
        designed_chains=("A", "C"),
        fixed_context_chains=("B",),
        preserved_metal_identity="ND",
        preserved_solvent_residue_count=1,
        redesigned_residue_ids=("A35", "C8"),
    ...
    command = build_round2_ligandmpnn_run_command(
...
def test_parse_and_summarize_round2_ligandmpnn_output(tmp_path: Path) -> None:
    ...
    candidate = LigandMPNNRound2SmokeCandidate(
        ...
        source_pdb_path=tmp_path / "example_seed.pdb",
        designed_chains=("A",),
        fixed_context_chains=(),
        preserved_metal_identity="ND",
        preserved_solvent_residue_count=1,
    ...
    summary_row, catalog_rows = summarize_round2_ligandmpnn_output(
        candidate=candidate,
    ...
    assert summary_row.seed_candidate_id == "example_seed"
    ...
    assert summary_row.preserved_metal_identity == "ND"
    ...
    assert catalog_rows[0].seed_candidate_id == "example_seed"
